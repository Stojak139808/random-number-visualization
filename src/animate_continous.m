function animate_continous(fun,n, pooling_intervals, seed, ax1, ax2, delay)
...
%get function handle from txt
fun_handle = str2func(fun);

%discretize distribution
density_function = divide_distribution(fun_handle, pooling_intervals);

%generate cummulatove disribution
cdf = CDF(density_function);

%values for plotting density function
plot_density = fun_handle(cdf(:,1));
...
for i = 1:n
    ...
    cla(ax1)
    hold(ax1,'on')
    plot(ax1,cdf(:,1), plot_density, 'Color', '#0072BD');
    ...
    stairs(ax1,cdf(:,1),cdf(:,2), 'Color', '#7E2F8E');
    hold(ax1,'off')
